...
### Parallel Agent Work with Git Worktrees

When multiple agents work simultaneously, **use git worktrees** to isolate work. This prevents agents from seeing each other's uncommitted changes.

#### Why Worktrees?

Without worktrees, agents working in the same directory see each other's uncommitted changes, leading to:
- Confusion about what code is "real" vs. in-progress
- Accidental inclusion of other agents' work in commits
- Difficulty knowing which changes belong to which task

#### Setup (Human or first agent)

Create a worktree for each parallel agent session:

```bash
# From the main project directory
git worktree add ../untitledgardenproject-agent-<task> -b feature/<task>

# Example:
git worktree add ../untitledgardenproject-agent-particles -b feature/particle-system
git worktree add ../untitledgardenproject-agent-refactor -b refactor/update-split
```

Each worktree:
- Has its own working directory (separate folder)
- Shares the same `.git` data (branches, commits, history)
- Can be on a different branch
- Shows only committed code from main + that agent's work

#### Agent Instructions (When Using a Worktree)

1. **Confirm your worktree**: Check you're in the correct directory (`pwd`)
2. **Start from main**: Your worktree should branch from latest main
3. **Work in isolation**: Other agents' uncommitted work won't affect you
4. **Commit frequently**: Your work becomes visible to others only when merged
5. **Merge to main**: When done, merge your branch in the main worktree or your worktree

#### Merging from a Worktree

```bash
# Option A: Merge from the worktree itself
cd ../untitledgardenproject-agent-<task>
git checkout main
git pull
git merge feature/<task>
git push
git checkout feature/<task>  # Switch back if continuing work

# Option B: Merge from the main worktree
cd ../untitledgardenproject  # Main worktree
git checkout main
git pull
git merge feature/<task>
git push
...
```bash
# Remove the worktree
git worktree remove ../untitledgardenproject-agent-<task>
...
If worktrees aren't set up, fall back to:

- Check TODO.md "In Progress" section first
- Avoid claiming the same files as other agents
- Pull before merging to catch conflicts early
- Be aware that you may see uncommitted changes from other sessions
