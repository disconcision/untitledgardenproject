...
import { lerpHexOklch, getDayNightBlend, getGoldenHourFactor } from "./oklch";
...
export function interpolateScheme(timeOfDay: number): ColorScheme {
  ...
  if (goldenHour > 0.1) {
    ...
    baseScheme.bgMid = lerpHexOklch(
      baseScheme.bgMid,
      goldenHourTints.pinkishHue,
      goldenHour * 0.25
    );
    baseScheme.bgPale = lerpHexOklch(
      baseScheme.bgPale,
      goldenHourTints.goldenGlow,
      goldenHour * 0.2
    );
  }
